...
public class Creature:MonoBehaviour
{
	...
	public void GetUserInputs(int idle1=0,int idle2=0,int idle3=0,int idle4=0,int eat=0,int drink=0,int sleep=0,int rise=0)
	{
		...
		{
			//Run key
			bool run=ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) ? true : false;

			//Attack key
			if(canAttack)
			{
				if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse0)) { behaviorCount=500; behavior="Hunt"; anm.SetBool("Attack",true); }
				else anm.SetBool("Attack",false);
			}

			//Crouch key
			if(main.useIK&&ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl)) { crouch=crouch_Max*size; onCrouch=true; }
			else onCrouch=false;
			...
			if(canFly|canSwim)
			{
				if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
				{
					...
					if(ControlFreak2.CF2Input.GetAxis("Mouse Y")!=0&&anm.GetInteger("Move")==3) //Pitch with mouse if is moving
						anm.SetFloat("Pitch",ControlFreak2.CF2Input.GetAxis("Mouse Y"));
					else if(ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl)) anm.SetFloat("Pitch",1.0f);
					else if(ControlFreak2.CF2Input.GetKey(KeyCode.Space)) anm.SetFloat("Pitch",-1.0f);
				}
				else
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl)) anm.SetFloat("Pitch",1.0f);
					else if(ControlFreak2.CF2Input.GetKey(KeyCode.Space)) anm.SetFloat("Pitch",-1.0f);
			...
			//Jump
			if(canJump&&ControlFreak2.CF2Input.GetKey(KeyCode.Space)&&!onJump) anm.SetInteger("Move",3);
			//Move
			else if(ControlFreak2.CF2Input.GetAxis("Horizontal")!=0|ControlFreak2.CF2Input.GetAxis("Vertical")!=0)
			{
				//Flying/swim
				if(canSwim|(canFly&&!isOnGround))
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
					{
						if(ControlFreak2.CF2Input.GetAxis("Vertical")<0) anm.SetInteger("Move",-1); //Backward
						else if(ControlFreak2.CF2Input.GetAxis("Vertical")>0) anm.SetInteger("Move",3); //Forward
						else if(ControlFreak2.CF2Input.GetAxis("Horizontal")>0) anm.SetInteger("Move",-10); //Strafe-
						else if(ControlFreak2.CF2Input.GetAxis("Horizontal")<0) anm.SetInteger("Move",10); //Strafe+
					...
					else
					{
						...
						float ang=main.transform.eulerAngles.y+Mathf.Atan2(ControlFreak2.CF2Input.GetAxis("Horizontal"),ControlFreak2.CF2Input.GetAxis("Vertical"))*Mathf.Rad2Deg;
				...
				else
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
					{
						if(ControlFreak2.CF2Input.GetAxis("Vertical")>0&&!run) anm.SetInteger("Move",1); //Forward
						else if(ControlFreak2.CF2Input.GetAxis("Vertical")>0) anm.SetInteger("Move",2); //Run
						else if(ControlFreak2.CF2Input.GetAxis("Vertical")<0) anm.SetInteger("Move",-1); //Backward
						else if(ControlFreak2.CF2Input.GetAxis("Horizontal")>0) anm.SetInteger("Move",-10); //Strafe-
						else if(ControlFreak2.CF2Input.GetAxis("Horizontal")<0) anm.SetInteger("Move",10); //Strafe+
						anm.SetFloat("Turn",transform.eulerAngles.y+ControlFreak2.CF2Input.GetAxis("Mouse X")*22.5f);//Mouse turn
					}
			...
			else if(ControlFreak2.CF2Input.GetKey(KeyCode.F)) //Eat / Drink
			{
				...
				if(isOnWater)
				{
					...
					if(ControlFreak2.CF2Input.GetKeyUp(KeyCode.F)) posTGT=Vector3.zero;
