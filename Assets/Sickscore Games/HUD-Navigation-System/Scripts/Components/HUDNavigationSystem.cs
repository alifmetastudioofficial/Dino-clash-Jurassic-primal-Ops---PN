...
namespace SickscoreGames.HUDNavigationSystem
{
	...
	public class HUDNavigationSystem : MonoBehaviour
	{
		...
		[Tooltip("If true, minimap image stays fixed (no pan / zoom / rotate with movement). Player icon follows PlayerController; POI markers use world positions on the map.")]
		public bool minimapStopMapMovement = true;
		...
		protected virtual void UpdateAllComponents ()
		{
			...
			if (useMinimap && minimapProfile != null && PlayerController != null)
				_HUDNavigationCanvas.UpdateMinimap (rotationReference, minimapMode, PlayerController, minimapProfile, minimapScale, minimapStopMapMovement, minimapShape);
		}
		...
		void UpdateMinimapElement (HUDNavigationElement element, Vector3 screenPos, float distance)
		{
			...
			Vector3 posOffset = element.GetPositionOffset (PlayerController.position);
			Vector3 markerPos;
			if (minimapStopMapMovement) {
				Vector3 worldDelta = element.transform.position - minimapProfile.MapBounds.center;
				markerPos = new Vector3 (worldDelta.x * unitScale.x, worldDelta.z * unitScale.y, 0f) * minimapScale;
			} else {
				markerPos = new Vector3 (posOffset.x * unitScale.x, posOffset.z * unitScale.y, 0f) * minimapScale;
			}

			// adjust marker position when using minimap rotation mode (map scrolls/rotates with player; skip when map is frozen)
			if (minimapMode == MinimapModes.RotateMinimap && !minimapStopMapMovement)
