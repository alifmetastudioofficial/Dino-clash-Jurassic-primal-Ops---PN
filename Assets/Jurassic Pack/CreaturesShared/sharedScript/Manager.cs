...
public class Manager : MonoBehaviour
{
  ...
  const string TIP1=
	...
	[Header("LEGACY ONGUI")]
	[Tooltip("Master switch for legacy OnGUI (debug/old UI). Default OFF.")]
	public bool enableLegacyOnGUI=false;
	[SerializeField] bool showGUI=true;
	[SerializeField] bool showFPS=true;
	[Tooltip("Show gameplay FPS label independent from legacy OnGUI panels.")]
	public bool showGameFPS=false;
	[Header("CANVAS HUD")]
	[Tooltip("Canvas HUD for current player creature needs (health/stamina/food/water).")]
	public bool showPlayerNeedsHUD=false;
	[Tooltip("Optional reference to PlayerNeedsHUD in scene.")]
	public PlayerNeedsHUD playerNeedsHUD;
	[Tooltip("Optional: player death revive UI (Canvas). Auto-assigned if present in scene.")]
	public PlayerReviveUI playerReviveUI;
  ...
  [Header("PATHFINDING (Creature.FindPath)")]
  [Tooltip("Linecast sirf in layers par hit karega — path inhi ke darmiyan 'khula' maana jayega. Terrain + Default select karo; rocks ko alag layer par rakho aur yahan se hata do.")]
  public LayerMask pathfindingLinecastMask = -1;
    [Header("GROUND DETECTION")]
    [Tooltip("Layers jin par creature khara ho sakta hai: Terrain, Ground, WalkableRock, etc.")]
  ...
  [Tooltip("The maximium walkable slope before the creature start slipping.")]
   ...
   // public float aiMaxSlope = 0.75f;
   // [Range(0.1f, 1000.0f)] public float playerMaxSlope = 1.2f;
     [Range(0.1f, 1000.0f)]public float MaxSlope=0.75f;
    [Tooltip("Water plane altitude")]
  public float waterAlt=55;
  [Tooltip("Blood particle for creatures")]
  public ParticleSystem blood;

	public List<GameObject> creaturesList, playersList; //list of all creatures/players in game
  ...
  #region START
	void Awake()
	{
		...
		if(useManager)
		{
			ControlFreak2.CFCursor.visible=false; ControlFreak2.CFCursor.lockState=CursorLockMode.Locked;
	  ...
	  if (pathfindingLinecastMask.value == 0)
		  pathfindingLinecastMask = -1; // Nothing = fallback: all layers (purana behaviour)
  }
  #endregion
  ...
  void Update()
	{
		...
		//Fps counter
		if(showFPS || showGameFPS) { frame += 1.0f; timer += Time.deltaTime; if(timer>=1.0f) { fps=frame; timer=0.0f; frame=0.0f; } }
		//Application.targetFrameRate=FPS_Limit;
		

		// Mouse movement / wheel sirf Control Freak 2 se (mobile touch), PC mouse nahi
		mouseX=ControlFreak2.CF2Input.GetAxis("Mouse X");
		mouseY=ControlFreak2.CF2Input.GetAxis("Mouse Y");
		wheel=ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel");
		horizontal=ControlFreak2.CF2Input.GetAxis("Horizontal")+Input.GetAxis("Horizontal");
		vertical=ControlFreak2.CF2Input.GetAxis("Vertical")+Input.GetAxis("Vertical");
		// Mouse buttons sirf Control Freak 2 se (mobile) – keyboard mouse click ignore
		mouse1=ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1);
		mouse2=ControlFreak2.CF2Input.GetKey(KeyCode.Mouse2);
		space=ControlFreak2.CF2Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.Space);
		shift=ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.LeftShift);
		control=ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.LeftControl);
		keyC=ControlFreak2.CF2Input.GetKeyDown(KeyCode.C) || Input.GetKeyDown(KeyCode.C);
		keyX=ControlFreak2.CF2Input.GetKeyDown(KeyCode.X) || Input.GetKeyDown(KeyCode.X);
		keyY=ControlFreak2.CF2Input.GetKeyDown(KeyCode.Y) || Input.GetKeyDown(KeyCode.Y);
		escape=ControlFreak2.CF2Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.Escape);

		
		//Lock/Unlock cursor
		if(Application.isEditor)
		{
			if(escape && toolBarTab==-1) { ControlFreak2.CFCursor.lockState=CursorLockMode.None; toolBarTab=1; }
			else if(escape && toolBarTab!=-1) { ControlFreak2.CFCursor.lockState=CursorLockMode.None; toolBarTab=-1; }
			else if(toolBarTab==-1) ControlFreak2.CFCursor.lockState=CursorLockMode.Locked;
		}
		else
		{
			if(ControlFreak2.CFCursor.lockState==CursorLockMode.None && escape) ControlFreak2.CFCursor.lockState=CursorLockMode.Locked;
			else if(escape) ControlFreak2.CFCursor.lockState=CursorLockMode.None;
		}
		...
		// Canvas needs HUD visibility.
		if (playerNeedsHUD != null)
		{
			playerNeedsHUD.manager = this;
			playerNeedsHUD.SetVisible(showPlayerNeedsHUD);
		}

		if (playerReviveUI != null)
			playerReviveUI.manager = this;
	}
	...
	void FixedUpdate()
	{
		...
		//	if(ControlFreak2.CFCursor.lockState==CursorLockMode.Locked | mouse2)
		//	{
  ...
  //                  vx +=mouseX*sensivity; //rotate cam X axe
		...
		//		}
		//		else if(!mouse2 | ControlFreak2.CFCursor.lockState!=CursorLockMode.Locked)
		//		{
  //   //                   Debug.Log("Follow camera2");
  //   //                   vx +=mouseX*sensivity; //rotate cam X axe
		//			//vy=Mathf.Clamp(invertYAxis?vy-mouseY*sensivity:vy+mouseY*sensivity, -90f, 90f); //rotate cam Y axe
		//		}
		//	}
  ...
  void OnGUI ()
	{
		...
		// Independent gameplay FPS (works even when legacy GUI is off).
		if(showGameFPS)
		{
			GUI.color=Color.white;
			GUIStyle fpsStyle = new GUIStyle(GUI.skin.label);
			fpsStyle.fontSize = 24; // ~2x bigger for readability
			fpsStyle.fontStyle = FontStyle.Bold;
			fpsStyle.normal.textColor = Color.white;
			GUI.Label(new Rect(sw-170, 4, 160, 32), "FPS : " + fps, fpsStyle);
		}

		if(!enableLegacyOnGUI) return;
        ...
        GUI.color=Color.white;
			...
			bool fullScreen=ControlFreak2.CFScreen.fullScreen; fullScreen= GUI.Toggle (new Rect((sw/2)-220, (sh/2)-80, 140, 20), fullScreen, " Fullscreen");
			if(fullScreen!=ControlFreak2.CFScreen.fullScreen) ControlFreak2.CFScreen.fullScreen=!ControlFreak2.CFScreen.fullScreen;
			showFPS= GUI.Toggle (new Rect((sw/2)-220, (sh/2)-40, 140, 20), showFPS, " Show Fps");
			enableLegacyOnGUI= GUI.Toggle (new Rect((sw/2)-220, (sh/2)-20, 140, 20), enableLegacyOnGUI, " Enable OnGUI");
		...
		} else ControlFreak2.CFCursor.visible=false;
