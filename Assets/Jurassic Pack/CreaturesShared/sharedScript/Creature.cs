...
{
	public static event Action<Creature, bool> OnWaterStateChanged;
	public static event Action<Creature> OnAIDied;
	public static event Action<Creature> OnCreatureDisabled;
    public static event Action<Creature, Creature, float> OnCreatureDamaged;
    private bool _sideMissionEatTracked;
    private Creature _lastSideMissionEatenCreature;
    private bool _aiDeathEventFired = false;
	
    private void ApplyDamageFromAttacker(Creature attacker, float damageAmount)
    {
        if (attacker == null)
            return;

        if (FocusModeController.Instance != null &&
			FocusModeController.Instance.IsFocusActive &&
			FocusModeController.Instance.FocusTarget == this &&
			attacker != null &&
			!attacker.useAI)
			{
				damageAmount *= FocusModeController.Instance.damageMultiplier;
			}

        lastDamageAttacker = attacker;
        killedByPlayer = !attacker.useAI;

        float oldHealth = health;
        health = Mathf.Clamp(health - damageAmount, 0.0f, 100f);

        float actualDamage = Mathf.Max(0f, oldHealth - health);
        if (actualDamage > 0f)
        {
            OnCreatureDamaged?.Invoke(attacker, this, actualDamage);
        }
    }

    #region VARIABLES
    private bool _playerDeathLogged = false;
    [HideInInspector] public Creature lastDamageAttacker;
    [HideInInspector] public bool killedByPlayer;
    [Space(10)]
    [Header("AI Reward")]
    [Tooltip("AI marne par player ko kitna cash milega.")]
    public int cashPrice = 10;
    [Space(10)]
    [Header("Locked On Target Setting")]
	public Transform lockOnPoint;
    [Space(10)]
	[Header("ARTIFICIAL INTELLIGENCE")]
	public bool useAI=false;
	[Tooltip("If true, this AI can auto-join zone combat against other species.")]
	public bool IamFighter = false;
	[Range(0.0f,1.0f)] public float CombatFightingChance = 1.0f;
	[Tooltip("When player enters close distance, chance to aggro player (runtime roll).")]
	[Range(0.0f,1.0f)] public float ChanceOfFightWithPlayer = 0.5f;
	const string TIP1 =
	...
	[Tooltip("Player-controlled speed multiplier (affects all movement: walk, run, strafe, swim, fly). 1 = default.")]
	[Range(0.1f,3.0f)] public float playerSpeedMultiplier = 1.0f;
	[Tooltip("AI speed multiplier (affects all movement for AI-driven creatures). 1 = default.")]
	[Range(0.1f,3.0f)] public float aiSpeedMultiplier = 1.0f;

	[Space(5)]
	[Header("SOUND MULTIPLIERS")]
	[Tooltip("Player-controlled creature sound volume multiplier. 1 = default authored clip volume.")]
	[Range(0.0f,3.0f)] public float playerSoundMultiplier = 1.0f;
	[Tooltip("AI creature sound volume multiplier. 1 = default authored clip volume.")]
	[Range(0.0f,3.0f)] public float aiSoundMultiplier = 1.0f;
	[Tooltip("Player creature footstep channel multiplier (source[1]). 1 = default.")]
	[Range(0.0f,3.0f)] public float playerFootSoundMultiplier = 1.0f;
	[Tooltip("AI creature footstep channel multiplier (source[1]). 1 = default.")]
	[Range(0.0f,3.0f)] public float aiFootSoundMultiplier = 1.0f;

	[Header("INPUT DEADZONES (MOBILE)")]
	[Tooltip("Touch joystick se halka sa drift aa kar walk/turn jittrering kare to yahan deadzone se stabilize hota hai.")]
	public bool applyMoveDeadzoneOnTouch = true;
	[Tooltip("Joystick axis is value ko deadzone ke under ho to 0 consider kiya jayega.")]
	[Range(0.01f,0.5f)] public float moveDeadzone = 0.12f;

	[Space(5)]
	[Header("WALK JERK STABILIZER (PLAYER)")]
	[Tooltip("When player walk force becomes too small, clamp speed multiplier to avoid vibration/jerk (without changing animation speed).")]
	public bool clampWalkMinPlayerSpeedMultiplier = false;
	[Tooltip("If playerSpeedMultiplier is below this value, we clamp it ONLY for walk-type moves (force <= threshold).")]
	[Range(0.1f, 1.0f)] public float walkMinPlayerSpeedMultiplier = 0.4f;
	[Tooltip("Used to detect walk-type moves by Move(force) parameter. Walk calls are usually 50; run calls are usually 128.")]
	[Range(0f, 200f)] public float walkForceDetectThreshold = 60f;

	[Space(5)]
	[Header("PLAYER MULTIPLIERS")]
	[Tooltip("Only for player-controlled creature (selected + cameraMode != 0). 1 = default. <1 slower change, >1 faster change.")]
	[Range(0.1f, 5f)] public float playerNeedsMultiplier = 1f;
	[Tooltip("Food is se kam ho to health regenerate nahi; is threshold se neeche bhook mana kar health ghategi (stamina/water 0 par bhi ghategi).")]
	[Range(0.01f, 10f)] public float minFoodForHealthRegen = 0.1f;

	[Header("SPRINT STAMINA LOCK (PLAYER)")]
	[Tooltip("If stamina is <= this, player sprint will be forced OFF.")]
	[Range(0f, 100f)] public float staminaEmptyThreshold = 0f;
	[Tooltip("After stamina empties, sprint stays OFF until stamina refills above this value.")]
	[Range(0f, 100f)] public float staminaRefillToSprintThreshold = 5f;
	[HideInInspector] public bool sprintLockedByStamina = false;

	[Header("PLAYER GROWL")]
	[Tooltip("Optional: exact Animator state name to play when player presses Growl input (G key / CF2 button). Example: Rex|Idle1B")]
	public string playerGrowlAnimationName = "";

	[Header("PLAYER REVIVE")]
	[Tooltip("Optional: full Animator state name to CrossFade after revive (e.g. Dilo|IdleA). If empty, uses Idle integer + Move=0 only.")]
	public string playerReviveAnimatorStateName = "";

	[Header("LATCH ATTACK (player)")]
	[Tooltip("If true, this playable dino can latch onto big AI targets (see LatchAttackManager).")]
	public bool canLatchAttack = false;
	[HideInInspector] public bool latchAttackActive = false;

	[Header("LATCH ATTACK (AI / large target)")]
	[Tooltip("If true, players with canLatchAttack can latch here when within range.")]
	public bool latchAttackAcceptor = false;
	public Transform latchPointRight;
	public Transform latchPointLeft;
	[Tooltip("When playerReviveAnimatorStateName is empty, sets Animator integer 'Idle' to this index (most dinos use 0 for first idle).")]
	public int playerReviveIdleIndex = 0;
	[Space(5)]
    ...
    public float maxFlyHeight = 150f;
	...
	// Cached only when the creature has PlayerFootstepParticleManager (player-only).
	private PlayerFootstepParticleManager _footstepParticleManager;
	protected PlayerFootstepParticleManager FootstepParticleManager
	{
		get
		{
			if (_footstepParticleManager == null)
				_footstepParticleManager = GetComponent<PlayerFootstepParticleManager>();
			return _footstepParticleManager;
		}
	}
	...
	[HideInInspector] public bool isActive, isVisible, isDead, isConstrained, isOnLevitation;
	...
	private bool _waterEventInitialized;
	private bool _lastIsInWaterForEvent;
	private float _lastWaterEventTime;
	private int _waterExitStreak;
	private int _waterEnterStreak;
	/// <summary>Min seconds between water enter/exit events (spam guard).</summary>
	private const float WaterEventMinInterval = 0.20f;
	/// <summary>Meters above/below water plane for hysteresis bands.</summary>
	private const float WaterEventHysteresis = 0.20f;
	/// <summary>Consecutive frames required before firing enter/exit (stops COM spikes during fight/stack on another dino).</summary>
	private const int WaterEventDebounceFrames = 5;
	#endregion
	...
	void Start()
	{
		...
		loop=UnityEngine.Random.Range(0,100);//Randomise start action
	...
	/// <summary>
	/// Restore player-controlled creature after death (UI revive button). Resets stats, death flags, and animator/rigidbody.
	/// </summary>
	public void RevivePlayer(float healthAmount = 100f, float foodAmount = 100f, float waterAmount = 100f, float staminaAmount = 100f)
	{
        GameAnalytics.Event("player_revived",
        GameAnalytics.P("health", Mathf.RoundToInt(healthAmount)),
        GameAnalytics.P("food", Mathf.RoundToInt(foodAmount)),
        GameAnalytics.P("water", Mathf.RoundToInt(waterAmount)),
        GameAnalytics.P("stamina", Mathf.RoundToInt(staminaAmount)),
        GameAnalytics.P("species", specie));

        health = Mathf.Clamp(healthAmount, 0.01f, 100f);
		food = Mathf.Clamp(foodAmount, 0f, 100f);
		water = Mathf.Clamp(waterAmount, 0f, 100f);
		stamina = Mathf.Clamp(staminaAmount, 0f, 100f);
		...
		onReset = true;
		lastHit = 0f;
		sprintLockedByStamina = false;

		if (body != null)
		{
			body.maxDepenetrationVelocity = 5f;
			body.constraints = RigidbodyConstraints.FreezeRotationZ;
			body.velocity = new Vector3(body.velocity.x * 0.25f, Mathf.Max(0f, body.velocity.y), body.velocity.z * 0.25f);
		...
		if (anm != null)
		{
			...
			if (!useAI)
			{
				// Player revive: prefer explicit state, otherwise use per-dino default recover state "<Specie>|Die-".
				string reviveState = !string.IsNullOrEmpty(playerReviveAnimatorStateName)
					? playerReviveAnimatorStateName
					: (specie + "|Die-");

				if (!string.IsNullOrEmpty(reviveState) && anm.HasState(0, Animator.StringToHash(reviveState)))
					anm.CrossFade(reviveState, 0.1f, 0, 0f);
				else
				{
					anm.SetInteger("Idle", playerReviveIdleIndex);
					anm.Update(0f);
				}
			}
		}
        ...
        if (!useAI && SideMissionManager.Instance != null)
            SideMissionManager.Instance.NotifyRevivePlayer();

    }
	#endregion CREATURE SETUP FUNCTIONS
	...
	public void StatusUpdate()
	{
		...
		anm.speed=animSpeed;
		// Per-creature sound scaling (player vs AI) applied globally to this creature's AudioSources.
		// source[1] is used as footstep channel across creature controllers, so it gets an extra foot multiplier.
		if (source != null && source.Length > 0)
		{
			float soundMul = useAI ? aiSoundMultiplier : playerSoundMultiplier;
			soundMul = Mathf.Max(0f, soundMul);
			float footMul = useAI ? aiFootSoundMultiplier : playerFootSoundMultiplier;
			footMul = Mathf.Max(0f, footMul);
			for (int i = 0; i < source.Length; i++)
			{
				if (source[i] != null)
					source[i].volume = (i == 1) ? (soundMul * footMul) : soundMul;
			}
		}
		...
		if(health>0.0f)
		{
			_aiDeathEventFired=false;
			if(loop>100)
			{
				bool isPlayerControlled = (main && (transform.gameObject==main.creaturesList[main.selected].gameObject) && (main.cameraMode!=0));
				float m = isPlayerControlled ? playerNeedsMultiplier : 1f;

				// If player has PlayerNeedsApplier, let it handle needs (per-second) and keep AI as-is.
				if (isPlayerControlled)
				{
					PlayerNeedsApplier applier = GetComponent<PlayerNeedsApplier>();
					if (applier != null && applier.ShouldOverrideCreatureStatusNeedsForPlayer())
					{
						// Food < minFoodForHealthRegen = regen nahi + drain; uske baad sab OK tab +0.1.
						if(food<minFoodForHealthRegen|stamina==0.0f|water==0.0f) health=Mathf.Clamp(health-0.1f,0.0f,100f);
						else health=Mathf.Clamp(health+0.1f,0.0f,100f);

                        TrackPlayerDeathAnalytics();
				...
				if(canSwim)
				{
					if(anm.GetInteger("Move")!=0) food=Mathf.Clamp(food-(0.01f*m),0.0f,100f);
					if(isInWater|isOnWater) { stamina=Mathf.Clamp(stamina+(1.0f*m),0.0f,100f); water=Mathf.Clamp(water+(1.0f*m),0.0f,100f); }
					else if(canWalk) { stamina=Mathf.Clamp(stamina-(0.01f*m),0.0f,100f); water=Mathf.Clamp(water-(0.01f*m),0.0f,100f); }
					else { stamina=Mathf.Clamp(stamina-(1.0f*m),0.0f,100f); water=Mathf.Clamp(water-(1.0f*m),0.0f,100f); health=Mathf.Clamp(health-1.0f,0.0f,100f); }
				}
				else
				{
					if(anm.GetInteger("Move")!=0) { stamina=Mathf.Clamp(stamina-(0.01f*m),0.0f,100f); water=Mathf.Clamp(water-(0.01f*m),0.0f,100f); food=Mathf.Clamp(food-(herbivorous ? (0.1f*m) : (0.01f*m)),0.0f,100f); }
					if(isInWater) { stamina=Mathf.Clamp(stamina-(1.0f*m),0.0f,100f); health=Mathf.Clamp(health-1.0f,0.0f,100f); }
				}
		...
		else
		{
			...
			if(useAI&&!_aiDeathEventFired)
			{
				_aiDeathEventFired=true;
				Invoke(nameof(onaideadevent), 3f);
            }
    ...
    private void TrackPlayerDeathAnalytics()
    {
        if (!useAI && health <= 0.01f && !_playerDeathLogged)
        {
            _playerDeathLogged = true;

            GameAnalytics.Event("player_died",
                GameAnalytics.P("species", specie),
                GameAnalytics.P("food", Mathf.RoundToInt(food)),
                GameAnalytics.P("water", Mathf.RoundToInt(water)),
                GameAnalytics.P("stamina", Mathf.RoundToInt(stamina)));
        }

        if (health > 0.01f)
        {
            _playerDeathLogged = false;
        }
    }
    //Collisions
	...
	public void ManageCollision(Collision col,AudioSource[] source,AudioClip pain,AudioClip Hit_jaw,AudioClip Hit_head,AudioClip Hit_tail)
	{
		...
		if(col.transform.CompareTag("Creature"))
		{
			...
			else if(lastHit==0&&other.onAttack)
			{
				...
				if(col.collider.gameObject.name.StartsWith("jaw")) //bite damage
				{
					...
					source[0].pitch= UnityEngine.Random.Range(1.0f,1.5f); source[0].PlayOneShot(pain,1.0f);
					source[1].PlayOneShot(Hit_jaw, UnityEngine.Random.Range(0.1f,0.4f));
                    ApplyDamageFromAttacker(other, baseDamages);
				...
				else if(col.collider.gameObject.name.Equals("head")) //head damage
				{
                    ...
                    if (!herbivorous)
                        ...
                        ApplyDamageFromAttacker(other, baseDamages / 10f);
										 ...
										 Transform RArm1=null,Transform RArm2=null,Transform RArm3=null,Transform LArm1=null,Transform LArm2=null,Transform LArm3=null,float FeetOffset=0.0f)
	{
		...
		if(main.useRaycast)
		{
			...
			{
				if(Physics.Raycast(transform.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit hit,withersSize*1.5f, main.groundMask))
				{ posY=hit.point.y; normal=hit.normal; isOnGround=true; }
				else isOnGround=false;
			}
			else if(ikType>=IkType.SmBiped) // Biped
			{
				if(Physics.Raycast((transform.position+transform.forward*2)+Vector3.up,-Vector3.up,out RaycastHit hit,withersSize*2.0f, main.groundMask))
				{ posY=hit.point.y; normal=hit.normal; }
				if(Physics.Raycast(RLeg3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit BR,withersSize*2.0f, main.groundMask))
				{ isOnGround=true; BR_HIT=BR.point; BR_Norm=BR.normal; }
				else BR_HIT.y=-transform.position.y;
				if(Physics.Raycast(LLeg3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit BL,withersSize*2.0f, main.groundMask))
			...
			{
				isOnGround=false;
				if(Physics.Raycast(transform.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit hit,withersSize*4.0f, main.groundMask))
				{
					normal=hit.normal; isOnGround=true;
					if(Physics.Raycast(RArm3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit FR,withersSize*4.0f, main.groundMask))
					{ FR_HIT=FR.point; FR_Norm=FR.normal; }
					else { FR_Norm=hit.normal; FR_HIT.y=-transform.position.y; }
					if(Physics.Raycast(LArm3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit FL,withersSize*4.0f, main.groundMask))
					{ FL_HIT=FL.point; FL_Norm=FL.normal; }
					else { FL_Norm=hit.normal; FL_HIT.y=-transform.position.y; }
					if(Physics.Raycast(RLeg3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit BR,withersSize*4.0f, main.groundMask))
					{ BR_HIT=BR.point; BR_Norm=BR.normal; }
					else { BR_Norm=hit.normal; BR_HIT.y=-transform.position.y; }
					if(Physics.Raycast(LLeg3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit BL,withersSize*4.0f, main.groundMask))
			...
			{
				isOnGround=false;
				if(Physics.Raycast(RArm3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit FR,withersSize*2.0f, main.groundMask))
				{ FR_HIT=FR.point; FR_Norm=FR.normal; isOnGround=true; }
				else FR_HIT.y=-transform.position.y;
				if(Physics.Raycast(LArm3.position+Vector3.up*withersSize,-Vector3.up,out RaycastHit FL,withersSize*2.0f, main.groundMask))
		...
		// Water UI/events: COM alone jitters when stacked on another creature in water (fight) — use lower sample + debounce.
		if(!_waterEventInitialized)
		{
			_lastIsInWaterForEvent=isInWater;
			_waterEventInitialized=true;
			_waterExitStreak=0;
			_waterEnterStreak=0;
		}
		else
		{
			float comY=body.worldCenterOfMass.y;
			float feetY=transform.position.y-Mathf.Max(0.05f,size)*0.5f;
			float sampleY=Mathf.Min(comY,feetY);
			float enterY=waterY-WaterEventHysteresis;
			float exitY=waterY+WaterEventHysteresis;
			bool wantEnter=sampleY<enterY;
			bool wantExit=sampleY>exitY;

			if(_lastIsInWaterForEvent)
			{
				if(wantExit) { _waterExitStreak++; _waterEnterStreak=0; }
				else _waterExitStreak=0;

				if(_waterExitStreak>=WaterEventDebounceFrames&&Time.time-_lastWaterEventTime>=WaterEventMinInterval)
				{
					_lastIsInWaterForEvent=false;
					_lastWaterEventTime=Time.time;
					_waterExitStreak=0;
					OnWaterStateChanged?.Invoke(this, false);
				}
			}
			else
			{
				if(wantEnter) { _waterEnterStreak++; _waterExitStreak=0; }
				else _waterEnterStreak=0;

				if(_waterEnterStreak>=WaterEventDebounceFrames&&Time.time-_lastWaterEventTime>=WaterEventMinInterval)
				{
					_lastIsInWaterForEvent=true;
					_lastWaterEventTime=Time.time;
					_waterEnterStreak=0;
					OnWaterStateChanged?.Invoke(this, true);
				}
			}
		}
	...
	void OnDisable()
	{
		_waterEventInitialized=false;
		_lastWaterEventTime=0f;
		_waterExitStreak=0;
		_waterEnterStreak=0;
		_aiDeathEventFired=false;
		OnCreatureDisabled?.Invoke(this);
	}
	#endregion
	...
	{
		// Snapshot base force BEFORE any multipliers/attack modifications.
		float baseMoveForce = force;

		// Global per-creature speed multipliers (player vs AI).
		float speedMul = useAI ? aiSpeedMultiplier : playerSpeedMultiplier;

		// Stabilize slow player walk (run is usually 128, so we detect walk by base force).
		if (!useAI && !jump && clampWalkMinPlayerSpeedMultiplier && baseMoveForce <= walkForceDetectThreshold)
		{
			speedMul = Mathf.Max(speedMul, walkMinPlayerSpeedMultiplier);
		}
		force *= speedMul;
		...
		if(CanMoveHead)
		{
			...
			else
			{
				if(Mathf.RoundToInt(anm.GetFloat("Turn"))==Mathf.RoundToInt(transform.eulerAngles.y))
				{
					...
					{
						rndX=UnityEngine.Random.Range((int)-yaw_Max/2,(int)yaw_Max/2);
						rndY=UnityEngine.Random.Range((int)-pitch_Max/2,(int)pitch_Max/2);
					}
	...
	{
		// Latch: mat chhedo objTGT / behaviorCount (LatchAttackManager set karta hai).
		if (!latchAttackActive)
		{
		...
		{
			// Latch attack: try trigger first; coroutine sets latchAttackActive before first yield.
			if (LatchAttackManager.Instance != null)
				LatchAttackManager.Instance.TryTriggerFromPlayer(this);

			if (latchAttackActive)
			{
				// Move/Idle band; attack CF2 Fire axis + Mouse0 se LatchAttackManager
				anm.SetInteger("Move", 0);
				anm.SetInteger("Idle", 0);
				if (LatchAttackManager.Instance != null)
					LatchAttackManager.Instance.ProcessLatchAttackInput(this);
				return;
			}

			float m = playerNeedsMultiplier;

			// Sprint stamina lock (player only).
			if (stamina <= staminaEmptyThreshold)
				sprintLockedByStamina = true;
			else if (stamina >= staminaRefillToSprintThreshold)
				sprintLockedByStamina = false;

			//Run key (keyboard + mobile)
			bool runRequested =
				ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) ||
				Input.GetKey(KeyCode.LeftShift) ||
				SprintToggle.IsOn;

			if (sprintLockedByStamina)
			{
				// Keep UI consistent: force toggle off if it was on.
				if (SprintToggle.IsOn && SprintToggle.Instance != null)
					SprintToggle.Instance.SetOff();
			}

			bool run = runRequested && !sprintLockedByStamina;

			// Manual growl input (player only) - G key / CF2 mapped button.
			if (!string.IsNullOrEmpty(playerGrowlAnimationName) &&
				(ControlFreak2.CF2Input.GetKeyDown(KeyCode.G) || Input.GetKeyDown(KeyCode.G)))
			{
				// Force a growl/roar animation without breaking existing state machine.
				behavior = "Player";
				behaviorCount = 0;
				anm.SetBool("Attack", false);
				anm.SetInteger("Move", 0);
				anm.CrossFade(playerGrowlAnimationName, 0.1f);
			}
			...
			if (canAttack)
			{
                ...
                // Attack sirf Control Freak se mouse click (mobile) – PC mouse click ignore
                if (ControlFreak2.CF2Input.GetKey(KeyCode.Mouse0))
				{
					behaviorCount = 500; behavior = "Hunt"; anm.SetBool("Attack", true);
                        if (LockOnManager.Instance != null)
                            LockOnManager.Instance.TryLockFromAttack();
			...
			//Crouch key
			if (main.useIK&&(ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.LeftControl))) { crouch=crouch_Max*size; onCrouch=true; }
			else onCrouch=false;

			//Fly/swim up/down key
			if(canFly|canSwim)
			{
				// Mouse1 sirf CF2 se (mobile)
				if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
				{
					float mx = ControlFreak2.CF2Input.GetAxis("Mouse X");
					float my = ControlFreak2.CF2Input.GetAxis("Mouse Y");
					anm.SetFloat("Turn",transform.eulerAngles.y+mx*22.5f);//Mouse turn
					if(my!=0&&anm.GetInteger("Move")==3) //Pitch with mouse if is moving
						anm.SetFloat("Pitch",my);
					else if(ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.LeftControl)) anm.SetFloat("Pitch",1.0f);
					else if(ControlFreak2.CF2Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.Space)) anm.SetFloat("Pitch",-1.0f);
				}
				else
				{
                    //amcode
                    if (canFly)
					{
						if (transform.position.y > maxFlyHeight)
						{
							...
							// optional: upward movement rok do
							//Pitch = Mathf.Min(Pitch, 0f);
						}

					}
                    //amcode warnnings to logs
                    if (ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.LeftControl)) anm.SetFloat("Pitch", 1.0f);
					else if (ControlFreak2.CF2Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.Space)) anm.SetFloat("Pitch", -1.0f);
			...
			//Jump
			if(canJump&&(ControlFreak2.CF2Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.Space))&&!onJump) anm.SetInteger("Move",3);
			//Move
			else if(ControlFreak2.CF2Input.GetAxis("Horizontal")+Input.GetAxis("Horizontal")!=0|ControlFreak2.CF2Input.GetAxis("Vertical")+Input.GetAxis("Vertical")!=0)
			{
				// Mobile joystick deadzone: small drift won't keep toggling Move state.
				float h = ControlFreak2.CF2Input.GetAxis("Horizontal") + Input.GetAxis("Horizontal");
				float v = ControlFreak2.CF2Input.GetAxis("Vertical") + Input.GetAxis("Vertical");
				if (applyMoveDeadzoneOnTouch && Input.touchCount > 0)
				{
					if (Mathf.Abs(h) <= moveDeadzone) h = 0f;
					if (Mathf.Abs(v) <= moveDeadzone) v = 0f;
				}

				// If deadzone kills input completely, treat it as stop.
				if (h == 0f && v == 0f)
				{
					anm.SetInteger("Move", 0);
				}
				...
				if(canSwim|(canFly&&!isOnGround))
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
					{
						if(v<0) anm.SetInteger("Move",-1); //Backward
						else if(v>0) anm.SetInteger("Move",3); //Forward
						else if(h>0) anm.SetInteger("Move",-10); //Strafe-
						else if(h<0) anm.SetInteger("Move",10); //Strafe+
					...
					else
					{
						...
						float ang=main.transform.eulerAngles.y+Mathf.Atan2(h, v)*Mathf.Rad2Deg;
				...
				else
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
					{
						if(v>0&&!run) anm.SetInteger("Move",1); //Forward
						else if(v>0) anm.SetInteger("Move",2); //Run
						else if(v<0) anm.SetInteger("Move",-1); //Backward
						else if(h>0) anm.SetInteger("Move",-10); //Strafe-
						else if(h<0) anm.SetInteger("Move",10); //Strafe+
						anm.SetFloat("Turn",transform.eulerAngles.y+ControlFreak2.CF2Input.GetAxis("Mouse X")*22.5f);//Mouse turn
					}
					else
					{
						float ang=main.transform.eulerAngles.y+Mathf.Atan2(h, v)*Mathf.Rad2Deg;
			...
			else
			{
				...
				if((canSwim|canFly)&&!isOnGround)
				{
					if(canSwim&&anm.GetFloat("Pitch")!=0&&!ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1)) anm.SetInteger("Move",run ? 2 : 1);
					else anm.SetInteger("Move",0);
				}
				//Terrestrial
				else
				{
					if(ControlFreak2.CF2Input.GetKey(KeyCode.Mouse1))
					{
						if(ControlFreak2.CF2Input.GetAxis("Mouse X")>0) anm.SetInteger("Move",10); //Strafe- 
						else if(ControlFreak2.CF2Input.GetAxis("Mouse X")<0) anm.SetInteger("Move",-10); //Strafe+
						else anm.SetInteger("Move",0);
						anm.SetFloat("Turn",transform.eulerAngles.y+ControlFreak2.CF2Input.GetAxis("Mouse X")*22.5f);//Mouse turn
					}
			...
			if(canInvertBody&&ControlFreak2.CF2Input.GetKeyDown(KeyCode.R)) { if(onInvert) onInvert=false; else onInvert=true; }
			...
			if(ControlFreak2.CF2Input.GetKey(KeyCode.E))
			{
				...
				rndIdle=UnityEngine.Random.Range(1,idles+1);
			...
			else if(ControlFreak2.CF2Input.GetKey(KeyCode.F)) //Eat / Drink
			{
				...
				if(isOnWater&&useAI)
				{
					...
					if(water<100) { behavior="Water"; water=Mathf.Clamp(water+(0.05f*m),0.0f,100f); }
					if(ControlFreak2.CF2Input.GetKeyUp(KeyCode.H)) posTGT=Vector3.zero;
				...
				else if(posTGT!=Vector3.zero)
				{
                    ...
                    if (!useAI && SideMissionManager.Instance != null)
                    {
                        ...
                        Creature eatenCreature = null;

                        if (objTGT != null)
                            eatenCreature = objTGT.GetComponent<Creature>();

                        if (eatenCreature != null && eatenCreature.isDead)
                        {
                            if (_lastSideMissionEatenCreature != eatenCreature && !useAI && SideMissionManager.Instance != null)
                            {
                                _lastSideMissionEatenCreature = eatenCreature;
                                SideMissionManager.Instance.NotifyEatDeadDino();
                            }
                        }



                        //_sideMissionEatTracked = true;
                        // SideMissionManager.Instance.NotifyEatDeadDino();
                    }

                    if (food<100) food=Mathf.Clamp(food+(0.05f*m),0.0f,100f);
					if(water<25) water+=(0.05f*m);
					if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.F))
					{ 
							posTGT=Vector3.zero;


                        //_sideMissionEatTracked = false;
                    }
				}
                //nothing found
                else
                {
                    if (!_sideMissionEatTracked && !useAI && SideMissionManager.Instance != null)
                       // _sideMissionEatTracked = false;
            ...
            else if (ControlFreak2.CF2Input.GetKey(KeyCode.H)) //Eat / Drink
            {
               // if (posTGT == Vector3.zero) FindPlayerFood(); //looking for food
                                                              //Drink
                if (isOnWater&&!useAI)
			...
			//Rise
			else if(rise!=0&&ControlFreak2.CF2Input.GetKey(KeyCode.Space)) anm.SetInteger("Idle",rise);
            ...
            //Head move
            if (ControlFreak2.CF2Input.GetKey(KeyCode.Mouse2))
			{
				onHeadMove=true;
				headX=Mathf.Lerp(headX,Mathf.Clamp(headX-ControlFreak2.CF2Input.GetAxis("Mouse X"),-yaw_Max,yaw_Max),0.5f);
				headY=Mathf.Lerp(headY,Mathf.Clamp(headY+ControlFreak2.CF2Input.GetAxis("Mouse Y"),-pitch_Max,pitch_Max),0.5f);
			}
			...
			{ behavior="Repose"; stamina=Mathf.Clamp(stamina+(0.05f*m),0.0f,100f); }
    ...
    public void onaideadevent()
	{
        OnAIDied?.Invoke(this);
    }
    public void ResetSideMissionEatFlag()
    {
        _sideMissionEatTracked = false;
    }
    bool FindPlayerFood()
	...
	/// <summary>
	/// Player UI helper:
	/// Returns true only when this creature can currently eat a dead creature
	/// using the same near-range concept as player eat input logic.
	/// </summary>
	public bool CanPlayerEatDeadCreatureNow(float extraRange = 0f)
	{
		if (main == null || main.creaturesList == null || herbivorous)
			return false;
		if (Head == null)
			return false;

		float eatRange = Mathf.Max(0.1f, boxscale.z + Mathf.Max(0f, extraRange));
		foreach (GameObject o in main.creaturesList.ToArray())
		{
			if (o == null || o == gameObject)
				continue;

			if ((o.transform.position - Head.position).magnitude > eatRange)
				continue;

			Creature other = o.GetComponent<Creature>();
			if (other == null || !other.isDead)
				continue;

			// Keep movement-medium compatibility similar to food search logic.
			if (!canSwim && other.isInWater)
				continue;
			if (canSwim && !canWalk && !other.isInWater)
				continue;
    ...
    /// <summary>
    /// Player UI helper:
    /// Returns true only when this herbivore creature can currently eat a plant
    /// (terrain detail grass OR tree layer), using same logic as FindPlayerFood.
    /// </summary>
    public bool CanPlayerEatPlantNow(float extraRange = 0f)
    {
        if (main == null || !herbivorous)
            return false;
        if (Head == null || main.t == null)
            return false;

        float eatRange = Mathf.Max(0.1f, boxscale.z + Mathf.Max(0f, extraRange));

        // Large creature — check tree layer (Physics sphere)
        if (withersSize > 8)
        {
            return Physics.CheckSphere(Head.position, withersSize + extraRange, main.treeLayer);
        }
        // Small/medium creature — check terrain detail layer (grass)
        else
        {
            float x = ((transform.position.x - main.t.transform.position.x) / main.tdata.size.z * main.tres);
            float y = ((transform.position.z - main.t.transform.position.z) / main.tdata.size.x * main.tres);

            int detailWidth = main.tdata.detailWidth;
            int detailHeight = main.tdata.detailHeight;

            int ix = Mathf.Clamp((int)x, 0, detailWidth - 1);
            int iy = Mathf.Clamp((int)y, 0, detailHeight - 1);

            for (int layer = 0; layer < main.tdata.detailPrototypes.Length; layer++)
            {
                if (main.tdata.GetDetailLayer(ix, iy, 1, 1, layer)[0, 0] > 0)
                    return true;
            }
        }
        return false;
    ...
    public void AICore(int idle1=0,int idle2=0,int idle3=0,int idle4=0,int eat=0,int drink=0,int sleep=0)
	{
		...
		if(posTGT==Vector3.zero)
		{
			if(nextPath>=pathEditor.Count) nextPath=0; //reset path list
																								 // edited path 
			if(pathEditor.Count>0&&UnityEngine.Random.Range(0,100)<pathEditor[nextPath].priority)
			...
			// look for water
			else if(canWalk&&UnityEngine.Random.Range(0,75)>water) { FindWater(); behaviorCount=4000; }
			// look for food or prey
			else if(UnityEngine.Random.Range(0,75)>food)
			{
				if(!herbivorous) { if(!FindFood()) FindPrey(); }
				else FindFood(); behaviorCount=4000;
			}
			// to sleep
			else if(!canSwim&&	UnityEngine.Random.Range(0,50)>stamina) { behavior="ToRepose"; FindPath(); behaviorCount=4000; }
			// look for friend
			else if(UnityEngine.Random.Range(0,5)==0) { FindFriend(); behaviorCount=4000; }
	...
	bool FindCustomTarget()
	{
		if(loop==0)
		{
			...
			for (int i = targetEditor.Count - 1; i >= 0; i--)
			{
				TargetEditor o = targetEditor[i];
				if(!o._GameObject) { targetEditor.RemoveAt(i); continue; } //gameobject no more exist
	...
	bool FindPath(bool invert=false)
	{
		...
		int pathMask = (main != null && main.pathfindingLinecastMask.value != 0) ? main.pathfindingLinecastMask.value : -1;

		// FLY TYPE
		if(canFly)
		{
			//altitude
			if(isOnGround) alt=UnityEngine.Random.Range(-90f,0);
			else if(posY<(lowAltitude ? -75 : -150)) { alt=UnityEngine.Random.Range(0,45f); }

			else alt=UnityEngine.Random.Range(-15f,0);
			//distance
			dist=UnityEngine.Random.Range(boxscale.z*10,boxscale.z*20);
			//direction 
			if(invert&&objTGT)
			{
				angle=UnityEngine.Random.Range(-15f-angleAdd,15f+angleAdd);
				TGT=transform.position+(Quaternion.Euler(alt,objTGT.transform.eulerAngles.y+angle,0)*Vector3.forward*dist);
			}
			else
			{
				angle=UnityEngine.Random.Range(-45f-angleAdd,45f+angleAdd);
			...
			{
				if(Physics.Linecast(TGT+Vector3.up*withersSize,TGT-Vector3.up*200,out hit, pathMask))
				{ if(hit.collider.gameObject.layer.Equals(main.waterLayer)) TGT=Vector3.zero; else TGT=hit.point; }
			}
			else if(Physics.Linecast(transform.position+Vector3.up*withersSize,TGT+Vector3.up*withersSize, pathMask)) TGT=Vector3.zero; //check for obstacle
		}
		// GROUND TYPE
		else if((canWalk&&!canSwim)|(canSwim&&canWalk&&!isOnWater&&!isInWater))
		{
			...
			if(invert&&objTGT)
			{
				dist=UnityEngine.Random.Range(boxscale.z*4,boxscale.z*10);
				angle=UnityEngine.Random.Range(-15-angleAdd,15+angleAdd);
				TGT=transform.position+(Quaternion.Euler(0,objTGT.transform.eulerAngles.y+angle,0)*Vector3.forward*dist);
			}
			else
			{
				dist=UnityEngine.Random.Range(boxscale.z*4,boxscale.z*10);
				angle=UnityEngine.Random.Range(-45-angleAdd,45+angleAdd);
				TGT=transform.position+(Quaternion.Euler(0,transform.eulerAngles.y+angle,0)*Vector3.forward*dist);
			}

			//check if position is reachable
			if(!Physics.Linecast(transform.position+Vector3.up*withersSize,TGT+Vector3.up*withersSize, pathMask)&&Physics.Linecast(TGT+Vector3.up*withersSize,TGT-Vector3.up*withersSize*2,out hit, pathMask))
		...
		else if(canSwim)
		{
			...
			if(isInWater)
			{
				if(lowAltitude) { if(isOnGround) alt=0; else alt=UnityEngine.Random.Range(0,45f); }
				else if(isOnWater) alt=UnityEngine.Random.Range(0,45f);
				else alt=UnityEngine.Random.Range(-60f,60f);
			}
			else alt=UnityEngine.Random.Range(0,45f);
			//direction and distance
			if(invert&&objTGT)
			{
				dist=UnityEngine.Random.Range(boxscale.z*10,boxscale.z*20);
			...
			{
				dist=UnityEngine.Random.Range(boxscale.z*4,boxscale.z*15);
				angle=UnityEngine.Random.Range(-45f-angleAdd,45f+angleAdd);
				if(isInWater)
				{
					if(lowAltitude) { if(isOnGround) alt=0; else alt=UnityEngine.Random.Range(0,45f); }
			...
			if(Physics.Linecast(transform.position+Vector3.up*withersSize,TGT+Vector3.up*withersSize, pathMask)) TGT=Vector3.zero; //check for obstacle
		}
	...
	void ExecuteBehavior(int idle1=0,int idle2=0,int idle3=0,int idle4=0,int eat=0,int drink=0,int sleep=0)
	{
		...
		if(loop==0) { rndMove=UnityEngine.Random.Range(0,100); if(idles_lenght>0) rndIdle=UnityEngine.Random.Range(0,idles_lenght+1); }
	...
	void AnmBattle(int rndMove,int idle1,int idle2,int idle3,int idle4,Creature other)
	{
		...
		if(canFly)
		{
			...
			{
				anm.SetBool("Attack",false); anm.SetInteger("Idle",UnityEngine.Random.Range(0,100)==0 ? 1 : 0);
			...
			else { anm.SetInteger("Move",2); anm.SetInteger("Idle",UnityEngine.Random.Range(0,100)==0 ? 1 : 0); }
		}
		//Terrestrial battles
		else if(!canSwim|(canSwim&&canWalk&&!isInWater))
		{
			...
			{
				anm.SetBool("Attack",false);
				if(other&&distTGT<actionDist*2.0f&&(rndMove>50&&other.rndMove<50)) { anm.SetInteger("Move",-1); AnmTurn(Vect.forward); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
				else if(distTGT<actionDist*2.0f&&rndMove>50) { anm.SetInteger("Move",-1); AnmTurn(Vect.forward); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
				else if(other&&rndMove<50&&other.rndMove<50) { anm.SetInteger("Move",2); AnmTurn(Vect.forward); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
				else if(!other&&rndMove<50) { anm.SetInteger("Move",2); AnmTurn(Vect.forward); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
				else if(!canSwim&&rndMove<75) { anm.SetInteger("Move",1); AnmTurn(Vect.strafe); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
				else { anm.SetInteger("Move",0); AnmTurn(Vect.forward); AnmIdles(rndIdle,idle1,idle2,idle3,idle4); }
			}
			else { anm.SetInteger("Move",2); anm.SetBool("Attack",false); AnmTurn(Vect.forward); anm.SetInteger("Idle",UnityEngine.Random.Range(0,10)==0 ? 1 : 0); }
		}
		//Water battles 
		else if(canSwim)
		{
