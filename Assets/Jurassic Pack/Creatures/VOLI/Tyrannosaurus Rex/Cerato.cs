  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Bigstep,Largesplash,Largestep,Idlecarn,Bite,Swallow,Sniff1,Rex1,Rex2,Rex3,Rex4,Rex5;
	...
	void PlaySound(string name, int time)
	{
		if(time==currframe && lastframe!=currframe)
		{
			...
			case "Step": source[1].pitch=Random.Range(0.75f, 1.25f);
				...
				if (FootstepParticleManager != null)
					FootstepParticleManager.SpawnFootstep(transform.position, isOnWater, isInWater);
    ...
    if(isOnGround | isInWater | isOnWater)
    {
      if(!isOnGround && !isInWater)
		...
		else { body.drag=4; body.angularDrag=4; }
      ApplyYPos();
    } else ApplyGravity();

		//Stopped
		if(OnAnm.IsName("Cerato|Idle1A") | OnAnm.IsName("Cerato|Idle2A") | OnAnm.IsName("Cerato|Die1") | OnAnm.IsName("Cerato|Die2"))
		{
      Move(Vector3.zero);
			if(OnAnm.IsName("Cerato|Die1")) { onReset=true; if(!isDead) { PlaySound("Atk", 2); PlaySound("Die", 12); } }
			else if(OnAnm.IsName("Cerato|Die2")) { onReset=true; if(!isDead) { PlaySound("Atk", 2); PlaySound("Die", 10); } }
		}

		//End Forward
		else if(OnAnm.normalizedTime > 0.5 && (OnAnm.IsName("Cerato|Step1+") | OnAnm.IsName("Cerato|Step2+") |
		        OnAnm.IsName("Cerato|ToIdle1C") | OnAnm.IsName("Cerato|ToIdle2B") | OnAnm.IsName("Cerato|ToIdle2D") | OnAnm.IsName("Cerato|ToEatA") |
		        OnAnm.IsName("Cerato|ToEatC") | OnAnm.IsName("Cerato|StepAtk1") | OnAnm.IsName("Cerato|StepAtk2")))
			PlaySound("Step", 9);

		//Forward
		else if(OnAnm.IsName("Cerato|Walk") | OnAnm.IsName("Cerato|WalkGrowl") | (OnAnm.normalizedTime < 0.5 &&
		   (OnAnm.IsName("Cerato|Step1+") | OnAnm.IsName("Cerato|Step2+") | OnAnm.IsName("Cerato|ToIdle2B") |
		   OnAnm.IsName("Cerato|ToIdle1C") | OnAnm.IsName("Cerato|ToIdle2D") | OnAnm.IsName("Cerato|ToEatA") | OnAnm.IsName("Cerato|ToEatC")) ) )
		{
      Move(transform.forward, 50);
			if(OnAnm.IsName("Cerato|WalkGrowl")) { PlaySound("Growl", 1); PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(OnAnm.IsName("Cerato|Walk")) { PlaySound("Step", 6); PlaySound("Step", 13); }
			else { PlaySound("Step", 8); PlaySound("Step", 12); }
		}

		//Run
		else if(OnAnm.IsName("Cerato|Run") | OnAnm.IsName("Cerato|RunGrowl") | OnAnm.IsName("Cerato|WalkAtk1") | OnAnm.IsName("Cerato|WalkAtk2") |
		   (OnAnm.normalizedTime < 0.6 && (OnAnm.IsName("Cerato|StepAtk1") | OnAnm.IsName("Cerato|StepAtk2"))))
		{
      roll=Mathf.Clamp(Mathf.Lerp(roll, spineX*5.0f, 0.05f), -20f, 20f);
			Move(transform.forward, 128);
			if(OnAnm.IsName("Cerato|RunGrowl")) { PlaySound("Growl", 1); PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(OnAnm.IsName("Cerato|Run")) { PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(OnAnm.IsName("Cerato|StepAtk1") | OnAnm.IsName("Cerato|StepAtk2")) { onAttack=true; PlaySound("Atk", 2); PlaySound("Bite", 5); }
			else { onAttack=true; PlaySound("Atk", 2); PlaySound("Step", 6); PlaySound("Bite", 9); PlaySound("Step", 13); }
		}

		//Backward
		else if((OnAnm.normalizedTime > 0.4 && OnAnm.normalizedTime < 0.8) && (OnAnm.IsName("Cerato|Step1-") | OnAnm.IsName("Cerato|Step2-") | OnAnm.IsName("Cerato|ToSleep2")))
		{
			Move(-transform.forward, 50);
			PlaySound("Step", 12);
		}
		...
		//Strafe/Turn right
		else if(OnAnm.IsName("Cerato|Strafe1-") | OnAnm.IsName("Cerato|Strafe2+"))
		{
    ...
    //Idle Attack
    else if(OnAnm.IsName("Cerato|IdleAtk1") | OnAnm.IsName("Cerato|IdleAtk2"))
		{ 
      ...
      PlaySound("Atk", 1); PlaySound("Step", 3); PlaySound("Bite", 6);
		...
		//Various
		else if(OnAnm.IsName("Cerato|EatA")) { onReset=true; isConstrained=true; PlaySound("Food", 4); PlaySound("Bite", 5); }
		else if(OnAnm.IsName("Cerato|EatB") | OnAnm.IsName("Cerato|EatC")) { onReset=true; isConstrained=true; }
		else if(OnAnm.IsName("Cerato|Sleep")) { onReset=true; isConstrained=true; PlaySound("Repose", 2); }
		else if(OnAnm.IsName("Cerato|ToSleep1") | OnAnm.IsName("Cerato|ToSleep2")) { onReset=true; isConstrained=true; }
		else if(OnAnm.IsName("Cerato|ToIdle2A")) { Move(Vector3.zero); PlaySound("Sniff", 1); }
		else if(OnAnm.IsName("Cerato|Idle1B")) { Move(Vector3.zero); PlaySound("Growl", 2); }
		else if(OnAnm.IsName("Cerato|Idle1C")) { Move(Vector3.zero); PlaySound("Sniff", 4); PlaySound("Sniff", 7); PlaySound("Sniff", 10);}
		else if(OnAnm.IsName("Cerato|Idle2B")) { Move(Vector3.zero); onReset=true; PlaySound("Bite", 4); PlaySound("Bite", 6); PlaySound("Bite", 8);}
		else if(OnAnm.IsName("Cerato|Idle2C")) { Move(Vector3.zero); PlaySound("Growl", 2); }
		else if(OnAnm.IsName("Cerato|Idle2D")) { Move(Vector3.zero); onReset=true; PlaySound("Atk", 2); }
		else if(OnAnm.IsName("Cerato|Die1-")) { isConstrained=true; PlaySound("Growl", 3); isDead=false; }
		else if(OnAnm.IsName("Cerato|Die2-")) { isConstrained=true; PlaySound("Growl", 3); isDead=false; }
