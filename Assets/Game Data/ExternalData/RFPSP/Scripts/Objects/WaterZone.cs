...
public class WaterZone : MonoBehaviour {
	...
	public GameObject UnderWaterDome;
	...
	void Start () {
        ...
        waterSplash = WaterEffects.waterSplash;
        rippleEffect = WaterEffects.rippleEffect;
        bubblesEffect = WaterEffects.bubblesEffect;
        splashTrail = WaterEffects.splashTrail;

       //amcodenew Physics.IgnoreCollision(myTransform.GetComponent<Collider>(), PlayerCollider, true);
		...
		if(useCausticsLight && causticsLight){
			...
			UnderWaterDome.gameObject.SetActive(false);
			UnderPlane.gameObject.SetActive(false);
		}
	...
	//	FPSWalkerComponent.holdingBreath = false;
		...
		if(frames.Length > 0){
			InvokeRepeating("NextFrame", 0.0f, 1.0f / fps);
		}
		if (frames.Length > 0)
		{
			InvokeRepeating("NextFrameuperlight", 0.0f, 1.0f / fps);
		}

	}
	public float CameraUpTransform;
	void Update(){
	
		

		if(!myTransform.GetComponent<Collider>().bounds.Contains(mainCamTransform.position - (mainCamTransform.up * CameraUpTransform))){	
			StopUnderwaterEffects();
		}else{

			Debug.LogError("i am under water");
	...
	void NextFrameuperlight()
	{
		if (!causticsLight.isActiveAndEnabled)
		{
			index = (index + 1) % frames.Length;
			causticsLight1.cookie = frames[index];
		}
	}
	...
	void OnTriggerEnter(Collider col){
        ...
        if ((colTag == "Player" && colTransform.position.y > myTransform.GetComponent<Collider>().bounds.max.y - 0.4f)
		&& lastSplashTime + 0.6f < Time.time)
			////&& (FPSWalkerComponent.jumping || !FPSWalkerComponent.grounded))//play splash effects for player if they jumped into water
   ...
   //     || col.gameObject.name == "Chest")
        
        {
            ...
            //foreach (Transform child in waterSplash.transform)
          ...
          //  FPSWalkerComponent.inWater = true;
            ...
            if (col.gameObject.GetComponent<Collider>().bounds.max.y - 0.95f <= myTransform.GetComponent<Collider>().bounds.max.y)
            {
              ...
              //  FPSWalkerComponent.swimming = true;
                ...
                {
                    //splashPos = new Vector3(col.transform.position.x, myTransform.GetComponent<Collider>().bounds.max.y + 0.01f, col.transform.position.z);
                    //waterSplash.transform.position = splashPos;
                    //foreach (Transform child in waterSplash.transform)
                    //{//emit all particles in the particle effect game object group stored in impactObj var
                    //    ParticleSystem partSys = child.GetComponent<ParticleSystem>();
            ...
            //    if (FPSWalkerComponent.inputY == 0 && FPSWalkerComponent.inputX == 0)
            //    {//player is treading water
            ...
            //        {
            //            rippleEffect.Emit(Mathf.RoundToInt(rippleEffect.emission.rateOverTime.constant));//emit the particle(s)
            ...
            //            splashTrail.Emit(Mathf.RoundToInt(splashTrail.emission.rateOverTime.constant));//emit the particle(s)
	...
	private void StartUnderwaterEffects(){
		...
		//	PlayerWeaponsComponent.waterMuzzleFlashColor.b = underwaterFogColor.b;
			...
			//if(waterPlane){//flip the water plane so we can see the surface from underwater
			//	waterPlane.gameObject.SetActive(true);
			//}
			
			if(useCausticsLight && causticsLight){
				causticsLight.gameObject.SetActive(true);
				UnderWaterDome.gameObject.SetActive(true);
				UnderPlane.gameObject.SetActive(true);
			}
		...
		//if(waterPlane && GameManager.GM.CurrentPlayer){
		//	waterPlane.position = new Vector3(GameManager.GM.CurrentPlayer.transform.position.x, 
		//	                                  underwaterYpos, 
		//	                                  GameManager.GM.CurrentPlayer.transform.position.z);
		//}
		...
		bubblesEffect.Emit(Mathf.RoundToInt(bubblesEffect.emission.rateOverTime.constant));
